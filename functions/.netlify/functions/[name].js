...
async function adminSchedules(context, supabase) {
  ...
  if (action === "restore") {
    const ids = [...new Set((Array.isArray(body.ids) ? body.ids : []).map(Number).filter(Number.isInteger))];
    if (!ids.length || ids.length > 200) return json(400, { status: "error", message: "Pilih jadwal arsip yang akan diaktifkan" });
    ...
    if (!archived.length) return json(400, { status: "error", message: "Jadwal arsip tidak ditemukan" });
    ...
    for (const item of archived) {
      const conflict = candidates.find((other) => Number(other.room_id) === Number(item.room_id) && other.day_name === item.day_name && item.period_start <= other.period_end && item.period_end >= other.period_start && isOverlap(item.start_time, item.end_time, other.start_time.slice(0, 5), other.end_time.slice(0, 5)));
      if (conflict) return json(409, { status: "error", message: `Tidak dapat diaktifkan: bentrok dengan ${conflict.subject}` });
...
async function createBooking(context, supabase) {
  ...
  if (!/^\d{2}:\d{2}$/.test(startTime) || !/^\d{2}:\d{2}$/.test(endTime) || startTime >= endTime ||
      (requestType === "single" && (startTime < "07:00" || endTime > "16:00"))) {
    return json(400, {
      status: "error",
      message: requestType === "single"
        ? "Booking sekali hanya tersedia pukul 07:00 - 16:00"
