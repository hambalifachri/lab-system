...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (participantNims.some(nim => !/^\d{11}$/.test(nim)) || participantNims.length !== participantCount) {
      return response(400, { status: "error", message: "Daftar peserta wajib diunggah dan jumlahnya harus sesuai CSV" });
    }
    ...
    if (error) {
      ...
      return response(500, {
        status: "error",
        message: `Pengajuan tidak dapat disimpan (diagnosis: ${error.constraint || error.code || "DB-UNKNOWN"}).`
      });
    }
